...
{This is TMDParser - modified TParser 10.1
 The only modification is added property CalcError
 instead of raising exceptions.
 Modification was done by Vit Kovalcik.
...
type
  ...
  TMDParserExceptionEvent = procedure (Sender: TObject; E: Exception) of object;



  TMDCustomParser = class(TComponent)
  private
    FCalcError:Boolean;
  private
  ...
  private
    ...
    FOnParserError : TMDParserExceptionEvent;
  ...
  published
    property CalcError:Boolean read FCalcError;  
    ...
    property OnParserError: TMDParserExceptionEvent read FOnParserError write FOnParserError;
  end;




  TMDParser = class(TMDCustomParser)
  public
...
procedure Register;
begin
  RegisterComponents('Samples', [TMDParser]);
end;
(*
{$IFDEF VER80}
  {$R *.D16}
...
{$IFNDEF UseMath}
    if arg1^ = 0 then
      ...
      Here can be error, due to no-weird-conditions-checking (what is ln(0)?)
      dest^ := exp(arg2^*ln(arg1^));
{$ELSE}
    If ((Frac (arg2^) = 0) OR (arg2^ > MaxInt) OR (arg2^ < MaxInt)) AND
       (arg1^ <= 0) then
      CalcErr:=True
    else
...
{                                                                }
{****************************************************************}
function TMDCustomParser.ParseExpression(const AnExpression: string):boolean;
var
...
constructor TMDCustomParser.Create(AOwner: TComponent);
begin
...
destructor TMDCustomParser.Destroy;
begin
  FreeExpression;
...
procedure TMDCustomParser.SetVar(const VarName: string; const Value: extended);
begin
  SetVariable(VarName, Value);
end;

function TMDCustomParser.SetVariable(VarName: string; const Value: extended): PParserFloat;
var
  i: integer;
begin
...
function TMDCustomParser.GetVariable(const VarName: string): extended;
var
  i: integer;
begin
  with FVariables do
...
procedure TMDCustomParser.AddFunctionOneParam(const AFunctionName: string; const Func: TMathProcedure);
begin
...
procedure TMDCustomParser.AddFunctionTwoParam(const AFunctionName: string; const Func: TMathProcedure);
begin
  if IsValidIdent(AFunctionName) then
...
procedure TMDCustomParser.ClearVariables;
var
  i: integer;
...
procedure TMDCustomParser.ClearVariable(const AVarName: string);
var
  index: integer;
begin
  with FVariables do
...
function TMDCustomParser.VariableExists(const AVarName: string): boolean;
var
  index: integer;
begin
...
procedure TMDCustomParser.ClearFunctions;
begin
  FunctionOne.Clear;
  FunctionTwo.Clear;
...
procedure TMDCustomParser.ClearFunction(const AFunctionName: string);
var
  index: integer;
begin
  with FunctionOne do
...
procedure TMDCustomParser.FreeExpression;
var
  LastOP,
  NextOP: POperation;
begin
...
procedure TMDCustomParser.SetExpression(const AnExpression: string);
begin
  ParseExpression(AnExpression); { this implies FExpression := AnExpression }
end;


function TMDCustomParser.CalcValue: extended;
var
  LastOP: POperation;
begin
  ...
  begin
    LastOP := FStartOperationList;
    FCalcError:=False;
    while (LastOP^.NextOperation <> nil) AND (not FCalcError) do
    begin
      with LastOP^ do
      begin
        LastOP^.CalcErr:=False;
        Operation(LastOP);
        FCalcError:=LastOP^.CalcErr;
        LastOP := NextOperation;
      end;
    end;
    If not FCalcError then
    begin
      LastOP^.CalcErr:=False;
...
{                                                                }
{****************************************************************}
constructor TMDParser.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
class function TMDParser.RemoveBlanks(const s: string): string;
